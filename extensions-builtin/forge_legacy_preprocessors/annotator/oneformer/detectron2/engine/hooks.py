    ...
    def after_step(self):
        ...
        if (self.trainer.iter + 1) % self._period == 0 or (
            ...
            if torch.npu.is_available():
                max_reserved_mb = torch.npu.max_memory_reserved() / 1024.0 / 1024.0
                reserved_mb = torch.npu.memory_reserved() / 1024.0 / 1024.0
                max_allocated_mb = torch.npu.max_memory_allocated() / 1024.0 / 1024.0
                allocated_mb = torch.npu.memory_allocated() / 1024.0 / 1024.0
                ...
                self._runs += 1
                if self._runs == self._max_runs:
                    mem_summary = torch.npu.memory_summary()
                    self._logger.info("\n" + mem_summary)
